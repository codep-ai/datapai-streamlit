...
def _ddb_table():
    if FILE_INGEST_META_STORE != "dynamodb":
        return None
    return boto3.resource("dynamodb").Table(FILE_INGEST_DDB_TABLE)
...
def fi_convert_structured_to_parquet_cleaned(
    ...
    cleaned_prefix = cleaned_prefix.rstrip("/") + f"/{table_name}/"
    cleaned_key = f"{cleaned_prefix}{table_name}.parquet"

    buf = io.BytesIO()
    df.to_parquet(buf, index=False)
    buf.seek(0)

    #s3.upload_fileobj(buf, bucket, cleaned_key)
...
{
  "type": "final_answer",
  "result": "Human-readable summary of what you did, including S3 paths, Glide/Iceberg tables, validation result, and metadata status."
}
